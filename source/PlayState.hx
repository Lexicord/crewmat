...
class PlayState extends MusicBeatState
{
	...
	var santa:FlxSprite;
	var bgcrew:FlxSprite;
	var table:FlxSprite;
	var sussy:FlxSprite;
	var notif:FlxSprite;

	var susmont:Int = 0;
	var drainmultipl:Float = 0;
	var impp:Character;
	var fc:Bool = true;
	...
	override public function create()
	{
				...
				case 'meeting':
					{
							defaultCamZoom = 1;
							curStage = 'meeting';
							var space:FlxSprite = new FlxSprite().loadGraphic(Paths.image('sus/meeting/space'));
							space.antialiasing = true;
							space.scrollFactor.set(0.9, 0.9);
							space.screenCenter();

							var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('sus/meeting/wall'));
							bg.antialiasing = true;
							bg.screenCenter();
							...
							table = new FlxSprite().loadGraphic(Paths.image('sus/meeting/meeting_table'));
							...
							bgcrew = new FlxSprite();
							bgcrew.frames = Paths.getSparrowAtlas('sus/meeting/fgcrew');
							bgcrew.animation.addByPrefix('idle', 'watcheeers', 24, false);
							bgcrew.screenCenter();
							bgcrew.antialiasing = true;
							bgcrew.y += 485;
							bgcrew.setGraphicSize(Std.int(bgcrew.width * 1.65));
				...
				case 'eletrical':
					{
							defaultCamZoom = 0.75;
							curStage = 'eletrical';
							var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('sus/eletrical/wall'));
							bg.antialiasing = true;
							bg.screenCenter();

							//THIS IS A VENT!!!!!!!!!!
						//	table = new FlxSprite().loadGraphic(Paths.image('sus/eletrical/totally_normal_vent'));
						//	table.antialiasing = true;
						//	table.x = dad.x;
						//	table.y = dad.y;
						//	table.x -= 115;
		...
		switch (SONG.player2)
		{
			...
			case 'pico':
				camPos.x += 600;
				dad.y += 300;
			case 'crew' | 'sus' | 'impostor' | 'mongus' | 'green' | 'duo':
				dad.y += 500;
			case 'wires':
				dad.y += 240;
				dad.x -= 590;
		...
		if (dad.curCharacter == 'duo')
			{
				dad2 = new Character(dad.x, dad.y, 'green'); // adds player 2
				add(dad2);
				dad2.x -= 300;
				dad2.y -= 135;
			}
			...
			switch (curStage){
				case 'meeting':
					...
					add(bgcrew);
				case 'eletrical':
					impp = new Character(1000, 5000, 'sus'); // SUS
					add(impp);
				case 'skeld':
					add(sussy);
			}	
...
//			#end
	...
	*/
	...
	override function stepHit()
	{
		...
		if (curSong.toLowerCase() == 'suspicious'){
			if (curStep == 1282)
			{
				notif = new FlxSprite();
				notif.frames = Paths.getSparrowAtlas('sus/Meeting');
				notif.animation.addByPrefix('waa', 'emergency intro', 24, false);
				notif.screenCenter();
				notif.cameras = [camHUD];
				notif.antialiasing = true;
				add(notif);
				FlxG.sound.play(Paths.sound('meeting'));
				notif.animation.play('waa');
				notif.animation.finishCallback = function(hello:String)
					{
						remove(notif);
		...
		if (curSong.toLowerCase() == 'vented')
			{
				switch (curStep){
					case 1672 | 1687 | 1704 | 1720 | 1736 | 1751 | 1768 | 1783 | 1799 | 1815 | 1831 | 1847 | 1864 | 1880 | 1895 | 1910 | 1927 | 1944 | 1960 | 1975 | 1991 | 2007 | 2023 | 2040 | 2056 | 2072 | 2087 | 2103 | 2120 | 2135 | 2151 | 2167 | 2183 | 2200 | 2215 | 2232:
						FlxG.camera.flash(FlxColor.RED, 0.4);
					case 1664: 
						drainmultipl = 0;
					case 1919: 
						defaultCamZoom = 0.75;
					case 1657: 
						FlxG.sound.play(Paths.sound('vent'));
				}
				if (curStep == 1663){
						dad.visible = false;
						var fakedad:FlxSprite = new FlxSprite(0, 0);
						fakedad.frames = Paths.getSparrowAtlas('sus/impasta');
						fakedad.antialiasing = true;
						fakedad.setPosition(dad.x, dad.y);
						fakedad.x -= 245;
						fakedad.y -= 50;
					//	camHUD.visible = false;
						fakedad.animation.addByPrefix('SUS SUS SUS SUS SUS', 'Kill >:)', 24, false);
					//	table.visible = false; // this is the vent
						add(fakedad);
						fakedad.animation.play('SUS SUS SUS SUS SUS');
						fakedad.animation.finishCallback = function(sussypenis:String)
							{
								remove(fakedad);
								defaultCamZoom = 3;
								var oldx = dad.x;
								var oldy = dad.y;
								remove(dad);
								dad = new Character(oldx, oldy, 'sus'); // SUS
								add(dad);
								dad.y += 200;
								iconP2.animation.play('sus');
								dad.visible = true;
							//	table.visible = true;
							}
						}
	...
	if (curSong.toLowerCase() == 'suspicious')
			{	
				switch (curStep){
					case 135 | 150 | 168 | 183 | 199 | 215 | 231 | 247:
						alarm();
	...
	*/
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'meeting':
				bgcrew.animation.play('idle');
			case 'skeld':
				sussy.animation.play('dance');
				bgcrew.animation.play('idle');
			case 'mall':
				if(FlxG.save.data.distractions){
